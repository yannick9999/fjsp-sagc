...
def plot_operation_graph(
    ...
    """
    Draw the operation precedence graph for one FJSP instance.

    Args:
        adj:             [n, n] predecessor adjacency (dense)
        opes_appertain:  [n] job index per operation
        nums_opes:       int, number of real operations
        labels:          [n] optional per-node labels (e.g. gate scores)
        title:           plot title
        save_path:       if set, save PNG to this path
        show:            if True, call plt.show()
        ope_ma_adj:      [n, M] operation-machine adjacency (optional)
        show_machines:   if True and ope_ma_adj given, draw machine nodes
        eligible_opes:   [n] bool, eligible nodes get green border
        completed_opes:  [n] bool, completed nodes get grey border
        running_opes:    [n] bool, running nodes get orange border
    """
    adj = _to_numpy(adj)
