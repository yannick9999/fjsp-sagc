...
def plot_training_curves(data: dict, split: str, out_dir: Path):
    ...
    ax.set_ylabel("Validation Makespan / MWR\n(lower is better)", fontsize=12, labelpad=8)
    ax.set_title(f"Training Curves ({SPLIT_LABELS[split]} Checkpoint Metric)",
    ...
    fig.tight_layout()
    fig.savefig(out_dir / "01_training_curves.png", dpi=300, bbox_inches="tight")
    plt.close(fig)
...
def plot_probability_of_improvement(data: dict, out_name: str, title: str, out_dir: Path,
...
def plot_scaling(data: dict, out_dir: Path):
    ...
    for b, means in data["baselines"].items():
        ax.plot(x_pos, means, linestyle="--", color=BASELINE_COLORS.get(b, "gray"),
                alpha=0.7, marker="x", label=BASELINE_LABELS.get(b, b), zorder=2)
    ...
    fig.tight_layout()
    fig.savefig(out_dir / "05_scaling.png", dpi=300, bbox_inches="tight")
    plt.close(fig)
...
def plot_efficiency(data: dict, out_dir: Path):
    ...
    fig.tight_layout()
    fig.savefig(out_dir / "07_efficiency.png", dpi=300, bbox_inches="tight")
    plt.close(fig)
    ...
    steps = [
        ...
        ("03 Performance Profiles",
         lambda: plot_performance_profiles(data["performance_profiles"], TEST_SIZES, {}, 3,
                                           ...
                                           f"Performance Profiles with 95% Bootstrap Confidence Bands{suffix}",
                                           (13, 12), out_dir)),
        ("03b Performance Profiles (Hurink)",
         lambda: plot_performance_profiles(data["performance_profiles_hurink"], HURINK_DATASETS, HURINK_LABELS, 3,
                                           ...
                                           f"Performance Profiles with 95% Bootstrap Confidence Bands (Hurink){suffix}",
                                           (13, 5), out_dir)),
        ("03c Performance Profiles (10x10)",
         lambda: plot_performance_profiles(data["performance_profiles"], ["10x10"], {}, 1,
                                           "03c_performance_profiles_10x10.png",
                                           f"Performance Profile with 95% Bootstrap Confidence Bands (10x10){suffix}",
                                           (5, 5), out_dir,
        ...
        ("04 Probability of Improvement",
         lambda: plot_probability_of_improvement(data["probability_of_improvement"],
                                                  ...
                                                  f"Probability of Improvement{suffix}", out_dir,
        ...
        ("04b Probability of Improvement (Hurink)",
         lambda: plot_probability_of_improvement(data["probability_of_improvement_hurink"],
                                                  ...
                                                  f"Probability of Improvement (Hurink){suffix}", out_dir)),
        ("05 Scaling",
         lambda: plot_scaling(data["scaling"], out_dir)),
        ("07 Efficiency",
